...
valvecount = 0
#  best valve flow
def bvf(vd, startvalve, timeleft, valveson, totalflow):
    ...
    if v[1] or v[0] == 0:    # valve already open, or 0 flow, get times to other nodes
        t1 = [bvf(vd, xx, timeleft-1, valveson, totalflow) for xx in v[2]]
        return max(t1)       
    else:
        # assume opening valve: remaining flow for this valve plus the rest
        if timeleft == 2:
            return v[0]+totalflow
        else:
            v[1] = 1
            t2 = [bvf(vd, xx, timeleft-2, valveson+1, totalflow+v[0]*(timeleft-1)) for xx in v[2]]
            return max(t2)
